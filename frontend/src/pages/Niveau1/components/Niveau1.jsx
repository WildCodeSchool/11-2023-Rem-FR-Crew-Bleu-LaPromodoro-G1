...
import AjoutIndice from "../components/AjoutIndice";
import "../styles/Niveau1.css";

function Niveau1() {
    const [inventaire, setInventaire] = useState([]);
    const [indicesAffiches, setIndicesAffiches] = useState([
        { id: 1, nom: 'Indice1', image: 'src/pages/Niveau1/assets/Tableau.png', x: 175, y: 250 },
        // autres indices...
    ]);
    ...
    const ajouterAuInventaire = indice => {
        if (!inventaire.find(item => item.id === indice.id)) {
            ...
            setIndicesAffiches(indicesAffiches.filter(item => item.id !== indice.id));
        }
    };
        ...
        <>
            <div className="background">
                {indicesAffiches.map(indice => (
                    <AjoutIndice key={indice.id} indice={indice} onAjouter={ajouterAuInventaire} />
                ))}
